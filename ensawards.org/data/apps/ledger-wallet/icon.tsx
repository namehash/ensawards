import React from "react";

import { cn } from "@/utils/tailwindClassConcatenation";

// TODO: Think how we can enhance the display of this icon on dark backgrounds (without hacky if-ology in the component)
const Icon = ({ className, ...props }: React.SVGProps<SVGSVGElement>) => (
  <svg
    version="1.1"
    id="Layer_1"
    xmlns="http://www.w3.org/2000/svg"
    xmlnsXlink="http://www.w3.org/1999/xlink"
    x="0px"
    y="0px"
    viewBox="0 0 768.91 669.35"
    xmlSpace="preserve"
    className={cn("p-0.5 bg-white rounded-lg", className)}
    {...props}
  >
    <path
      d="M0,479.29v190.06h289.22V627.2H42.14V479.29H0z M726.77,479.29V627.2H479.69v42.14h289.22V479.29H726.77z M289.64,190.06
	v289.22h190.05v-38.01H331.78V190.06H289.64z M0,0v190.06h42.14V42.14h247.08V0H0z M479.69,0v42.14h247.08v147.92h42.14V0H479.69z"
    />
  </svg>
);

export default Icon;
