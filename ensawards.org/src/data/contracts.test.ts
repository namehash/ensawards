...
describe("contracts data", () => {
  ...
  describe("CachedEnsProfile", () => {
    ...
    it("if defined, each of the forwardNames must be a non-empty normalized ENS name", () => {
      data.forEach((contract) => {
        if (contract.cachedEnsProfile !== null && contract.cachedEnsProfile.forwardNames !== undefined) {
    ...
    it("if ensMetadata is defined then either primaryName or forwardNames field must be defined and not null", () => {
      data.forEach((contract) => {
        if (contract.cachedEnsProfile !== null && contract.cachedEnsProfile.ensMetadata !== undefined) {
          expect(
              contract.cachedEnsProfile.primaryName !== null || contract.cachedEnsProfile.forwardNames !== undefined,
