...
describe("Best Practices data", () => {
    ...
    describe("Category data", () => {
        it("Should have valid and unique slugs", () => {
            ...
            expect(areStringsUnique(slugArray), `Slugs for best practice categories are not unique`).toEqual(true);
        });
    })
    ...
    describe("Best Practice data", () => {
        it("Should have valid and unique slugs", () => {
            data.forEach((category) => {
                ...
                category.bestPractices.forEach((bestPractice) => {
                    expect(isValidSlug(bestPractice.slug), `Slug={${bestPractice.slug}} is not valid`).toEqual(true);
        ...
        it("Should have valid and unique categorySlugs", () => {
            data.forEach((category) => {
                const slugArray: string[] = [];
                category.bestPractices.forEach((bestPractice) => {
                    expect(isValidSlug(bestPractice.categorySlug), `Slug={${bestPractice.categorySlug}} is not valid`).toEqual(true);

                    slugArray.push(bestPractice.categorySlug);
                })

                expect(areStringsUnique(slugArray), `CategorySlugs for best practices are not unique`).toEqual(true);
            })
        });
    })
});