...
export function Link({ href, className, children, ...props }: PropsWithChildren<LinkProps>) {
  return (
    ...
    >
      ...
      <span className="block max-w-0 group-hover:max-w-full transition-all duration-200 h-[1px] bg-current self-stretch" />
    </a>
  );
}
...
export function ExternalLinkWithIcon({
  ...
  iconSize = 16,
  children,
}: PropsWithChildren<ExternalLinkWithIconProps>) {
  return (
    <Link href={href} target="_blank" rel="noreferrer noopener" className={className}>
      {children}
      <ExternalLinkIcon
