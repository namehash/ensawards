...
import {Check, X as XIcon, CheckCheck} from "lucide-react";
...
export function ContractBadge({ensProfileData}: ContractBadgeProps) {
    return (
        <TooltipProvider delayDuration={1000} skipDelayDuration={0}>
            {
                ensProfileData === null ? <span
                    className={cn(
                        "w-fit flex flex-row flex-nowrap justify-center items-center gap-[6px] pl-[10px] pr-3 " +
                        "py-1 rounded-full text-xs leading-normal font-medium", "text-red-600 bg-[#DC26261A]"
                    )}>
        ...
        Unnamed
                    ...
                    className={cn(
                        "w-fit flex flex-row flex-nowrap justify-center items-center gap-[6px] pl-[10px] pr-3 " +
                        "py-1 rounded-full text-xs leading-normal font-medium", ensProfileData.primaryName !== null ? "text-emerald-600 bg-[#0596691A]" : "text-orange-600 bg-orange-100"
                    )}>
