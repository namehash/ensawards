...
const generateReferralLinkFormSchema = Yup.object().shape({
    "ethereum address": Yup.string().required("Referrer's Ethereum address is required").test({
        name: "address-test",
        test(value, ctx) {
...
export function ReferralLinkForm() {
        ...
        try {
            ...
            if (validationError instanceof Yup.ValidationError) {
                ...
                for (const err of validationError.inner) {
                    if (
                        err.path &&
                        Object.values(ENSAwardsReferralLinkFormFields).includes(err.path as ENSAwardsReferralLinkFormFields)
                    ) {
   ...
   return <div className={cn("w-full h-fit gap-12 p-5 pt-8 sm:p-8 box-border bg-neutral-50 rounded-2xl", verticalFlex)}>
       <div className={cn(verticalFlex, "gap-5")}>
           <div className="w-12 h-12 flex flex-col justify-center items-center bg-[rgba(0,82,204,0.20)] rounded-full"><LinkIcon size={20} className="text-blue-600 flex-shrink-0"/></div>
           <div className={cn(verticalFlex, "gap-3")}>
               <h3 className="text-2xl leading-normal font-semibold text-center">Generate your referral&nbsp;link</h3>
               <p className="text-base leading-normal font-normal text-muted-foreground text-center">No signups or waiting for approval. Enter your Ethereum address below to generate your referral link.</p>
           </div>
       </div>
       <form
           onSubmit={submitForm}
       ...
       >
           ...
           <div
               className={cn([
                   successfulFormSubmit ? "w-full h-fit flex flex-col items-start justify-start gap-3 transition-all duration-300" : "hidden",
               ])}
           >
               <div className="w-full h-fit flex flex-col items-start justify-start gap-2">
                   <p className="text-sm leading-none font-medium">Your referral link
                       for {truncateAddress(referrerAddress as Address)}:</p>
                   <div
                       className="w-full h-[38px] flex flex-row justify-start items-center gap-[5px] p-[5px] pl-3 box-border bg-neutral-100 border border-input rounded-md shadow-xs">
                       <p className="w-full text-sm leading-normal font-normal text-muted-foreground overflow-hidden text-nowrap">{generatedLink}</p>
               ...
               >
                   <RefreshIcon size={16}/>Generate another link
               </button>
           </div>
           <div           className={cn([
               successfulFormSubmit ? "hidden" : "transition-all duration-300 self-stretch flex flex-col justify-start items-start gap-5",
           ])}>
