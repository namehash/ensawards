...
import { sortBenchmarks } from "data/benchmarks/utils";
...
export const BenchmarkTechnicalDetails = ({
  ...
  const sortedAcceptanceTestBenchmarks = [...Object.entries(acceptanceTestBenchmarks)].sort(
    (
      [aAcceptanceTestSlug, aAcceptanceTestBenchmark],
      [bAcceptanceTestSlug, bAcceptanceTestBenchmark],
    ) => {
      const benchmarkDiff = sortBenchmarks(aAcceptanceTestBenchmark, bAcceptanceTestBenchmark);

      if (benchmarkDiff !== 0) {
        return benchmarkDiff;
      }

      return aAcceptanceTestSlug.localeCompare(bAcceptanceTestSlug);
    },
  );
    ...
    <div className="max-w-[716px] flex flex-nowrap flex-col justify-center items-start gap-6">
      ...
      <div className={technicalSectionContainerStyles}>
        ...
        <div className={cn(technicalSectionContainerStyles, "gap-6")}>
          {sortedAcceptanceTestBenchmarks.map(([acceptanceTestSlug, acceptanceTestBenchmark]) => {
              ...
              >
                <div className={cn(technicalSectionContainerStyles, "gap-1")}>
                  ...
                  {acceptanceTestBenchmark && (
                    <div className={cn(technicalSectionContainerStyles, "gap-1")}>
                      <p className={cn(technicalSectionHeaderStyles, "text-lg")}>Benchmark notes</p>
